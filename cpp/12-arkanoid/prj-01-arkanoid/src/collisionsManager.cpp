...
bool gm::chekBallCollisionsLimit(std::shared_ptr<gm::Platform>& paddle, std::shared_ptr<gm::Ball>& ball){
    ...
    if(ball->time().getElapsedTime() < sf::Time(sf::milliseconds(100))){
        return true;
    }
    ball->resetTime();
...
std::vector<gm::BallBrickCollision> gm::CheckCollisionsBetweenBallBricks(std::shared_ptr<gm::Ball>& ball, std::vector <std::shared_ptr<gm::ConstObject>>& bricks){
...
void gm::collisionsManager(std::shared_ptr<gm::Platform>& paddle, std::shared_ptr<gm::Ball>& ball, std::vector <std::shared_ptr<gm::ConstObject>>&  bricks, gm::Player& myPlayer)
{
