...
void se::TextUI::setAndShowResult(std::unique_ptr<se::ResultIF>& result) const
{
    if(result != nullptr){//vecResult.size()==0;
        auto vecResult = result->getResult();
        for(auto& res : vecResult){
            std::cout<<std::left <<" "<<std::setw(70) << res.first<<std::right << std::setw(10) << res.second << std::endl;
        }
    }else{
